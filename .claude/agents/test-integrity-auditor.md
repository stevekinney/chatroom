...
That is **Claude Code's own `edited_text_file` notice**. For your OWN out-of-band write — your own Bash `cp` restoring a file you had read — expect zero: two reviewers independently confirmed a subagent authoring its own out-of-band write gets no notice for it. That is not the same as "subagents never see one at all" — a round-7 subagent did receive one, for a file the orchestrating main session concurrently edited through an ordinary Edit call while the subagent had it open, a different shape from your own restore. Do not treat the absence of a notice as evidence your restore worked — the hash is what tells you that — and do not go looking for a count you were primed to expect.
